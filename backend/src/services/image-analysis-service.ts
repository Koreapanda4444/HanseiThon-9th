...
export async function analyzeWasteImage(imageDataUrl: string) {
      ...
      body: JSON.stringify({
        ...
        input: [{
          ...
          content: [
            {
              type: "input_text",
              text: "사진에서 버릴 수 있는 물품을 각각 찾아 한국어 이름, 분리배출 카테고리, 신뢰도, 사진 전체를 0부터 100으로 본 바운딩 박스, 실제 배출 방법을 반환하세요. 서로 겹친 물품도 구분하고 배경이나 가구는 제외하세요. 카테고리는 general, recycle, medicine, battery, clothes, cigarette, electronics 중 하나만 사용하세요.",
            },
  ...
  if (responseText.length > 512 * 1024) {
